...
uint256 hashGenesisBlock("0x1952e403e8d57b1802ce37b7cc95a371a5627cc74b2b52794339e7cf4ca95ac7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Insacoin: starting difficulty is 1 / 2^12
...
{
    // Insacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Insacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Insacoin: 3.5 days
static const int64 nTargetSpacing = 1.0 * 60; // Insacoin: 2.5 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Insacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1537258510;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087313109;
        ...
        assert(block.hashMerkleRoot == uint256("0xb453dee39da89d903c5b39154fa88e87939d19dc609b646030a6910ebea08a0b"));
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb8, 0xdd }; // Insacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("InsacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static InsacoinMiner(CWallet *pwallet)
{
    printf("InsacoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running InsacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("InsacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&InsacoinMiner, pwallet));
}
